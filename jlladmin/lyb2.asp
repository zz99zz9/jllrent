...
<thead>
          <tr > 
            ...
            <th width="50" class=""><strong>留言时间</strong></th>
       <!--     <th width="170"  class="hidden-phone"><strong>内容</strong></th>-->
          ...
          <%do while not rs.eof%>
          <tr >
            <td class=""><%=rs("xmx")%><%=rs("xmm")%></td>
            <td class=""><%=rs("tel")%></td>
            <td class="hidden-phone"><%=rs("mail")%></td>
...
if action="Del" then
conn.execute "delete from [msg] where lyid="&Trim(delid)
